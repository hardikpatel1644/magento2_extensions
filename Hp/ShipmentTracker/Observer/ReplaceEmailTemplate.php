...
namespace Hp\ShipmentTracker\Observer;
...
{
    /**
     * @var \Hp\ShipmentTracker\Helper\Data
     */
    private $trackerDataHelper;

    /**
     * ReplaceEmailTemplate constructor.
     * @param \Hp\ShipmentTracker\Helper\Data $shipperDataHelper
     */
    public function __construct(
        \Hp\ShipmentTracker\Helper\Data $shipperDataHelper
    ) {
    ...
    {
        if ($this->trackerDataHelper->getConfigValue('carriers/hptracker1/active')) {
            $blockentity = $observer->getBlock();
            if ($blockentity->getTemplate() == 'Magento_Sales::email/shipment/track.phtml') {
                $blockentity->setTemplate('Hp_ShipmentTracker::email/shipment/track.phtml');
            }
        }
    }
}
